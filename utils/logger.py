...
class Logger:
    ...
    def get_logs(self):
        return self.loss_train, self.loss_val, self.acc_train, self.acc_val, self.weights

    def restore_logs(self, logs):
        self.loss_train, self.loss_val, self.acc_train, self.acc_val, self.weights = logs
    ...
    def save_plt(self, hps, k):
        ...
        weights_plot_path = os.path.join(model_save_dir, "weights_plot_{}-fold.jpg".format(k))
        ...
        if z != 0:
            ...
            for w_s in self.weights:
                for c_w, class_weight in enumerate(w_s):
                    weights[c_w].append(class_weight)

            colors = ["red", "orange", "yellow", "green", "blue", "indigo", "darkviolet"]

            fig, ax = plt.subplots(1, figsize = (10, 10))

            bottom = len(weights[0])*[0]
            for c in range(len(labels)):
                if z == 1:
                    plt.bar(x_s, weights[c], width = 1, bottom = bottom, color = colors[c])
                else:
                    plt.bar(x_s, weights[c], width = 20, bottom = bottom, color = colors[c])

                bottom = [b + w for b, w in zip(bottom, weights[c])]
            ...
            plt.savefig(weights_plot_path, bbox_inches = "tight", pad_inches = 0)